...
#define DATABASE_URL "https://medilifi-a63d4-default-rtdb.firebaseio.com/"
...
void loop() {
  ...
  if (mySerial.available() > 0) {                  // Check if data is available to read
    ...
    if (Firebase.ready()) {
      FirebaseJson jsonData("{\"temperature\":temperatureC,\"heartRate\":heartRate,\"spO2\":spO2}");
      jsonData.set("vitals/check/", F("device"));
      jsonData.set(F("value/ts/.sv"), F("timestamp"));
      Serial.printf("Set json... %s\n", Firebase.RTDB.set(&fbdo, F("/vals/json"), &jsonData) ? "ok" : fbdo.errorReason().c_str());
    }
